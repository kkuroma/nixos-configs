...
- **Prompt caching** is llama.cpp's, not the router's: the router forwards bodies verbatim, so a repeated prefix is reused whether or not the client sets `cache_prompt` (llama.cpp defaults it on). At 16k tokens the next turn costs 0.38 s instead of 5.0 s on Gemma-4-26B, and 0.42 s instead of 17.4 s on Wordslop-Qwen3.6-27B.
  ...
  - **Cost per held conversation is per model and the spread is 5x.** A 16k-token conversation costs 268 MiB on Gemma-4-26B (~16 KiB/token) and 1250 MiB on Wordslop-Qwen3.6-27B (~75 KiB/token), both at `ctk/ctv = q4_0`. At `cram = 4096` Wordslop held two such conversations and a third made it thrash to zero reuse; 16384 held all six. Set `cram` per model when one model's KV is much heavier than the rest.
  - `--cache-reuse` is useless here — every model in the preset list uses sliding-window attention, so llama.cpp disables KV shifting (`cache_reuse is not supported by this context` in the journal). A prompt edited anywhere but the tail reprocesses from the edit onward.
  ...
  - **The budget is `cram = 16384` (16 GiB) in `presetGlobals`.** The router repo gained a typed `services.llama-router.promptCache = { ramMiB, enable, reuseChunk, checkpoints, checkpointMinStep }` that writes the same `[*]` keys, but this repo's flake input is pinned to a router commit without it. After the router change is pushed and `nix flake update llama-router` runs, replace the `cram` line with `promptCache.ramMiB = 16384;`. Precedence in the new module is model key > `presetGlobals` > `promptCache`, so both spellings produce the same INI and neither breaks the other.
