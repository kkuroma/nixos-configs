...
let
  ...
  starshipConfig = {
    ...
    python = {
      symbol = pythonIc;
      format = "( ${open "red"}${slot "red" "$symbol $virtualenv"}${close "red"})";
      detect_files = [];
      detect_extensions = [];
      detect_folders = [];
    };
    ...
    env_var.DEV_SHELL = {
      ...
      variable = "DEV_SHELL";
      symbol = "${shellIc} ";
      format = "( ${open "blue"}${slot "blue" "$symbol$env_value"}${close "blue"})";
    };

    container = {
      disabled = false;
      symbol = cubeIc;
      format = "( ${open "blue"}${slot "blue" "$symbol $name"}${close "blue"})";
    };
