...
let
  ...
  # Static palette for the noctalia-off case: only the non-standard color names the prompt
  # uses need defining; green/blue/red/yellow/cyan/black resolve to starship's built-ins.
  fallbackPalette = ''
    [palettes.fallback]
    overlay1 = "#7f849c"
    overlay2 = "#9399b2"
...
in
{
  ...
  '' else ''
    ...
    mkdir -p "$(dirname "$dest")"
    if [ "$(cat "$sentinel" 2>/dev/null)" != "${configFile}" ]; then
      # No noctalia → fully static palette (nothing rewrites this file afterward).
