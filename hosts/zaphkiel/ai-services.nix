...
let
  ...
  # my own llama cpp router frontend and scheduler layer
  ...
  # let nix set up the configuration, no hunting for the config files now
...
in
{
  # llama user to avoid running code as superuser
  users.users.llama = {
    isSystemUser = true;
    group = "llama";
    extraGroups = [ "video" "render" ]; # gpu access
  };
  users.groups.llama = {};
  # access to weights and embedding
  ...
  # llama router for openai compatible LLM calls
  ...
  # cockpit - the frontend for systemd services
  services.cockpit = {
    enable = true;
    settings.WebService = {
      AllowUnencrypted = "true";
      Origins = lib.mkForce "http://localhost:9090"; # only allow local access
    };
  };
