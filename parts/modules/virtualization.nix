...
lib.mkIf config.host.features.virtualization {
  ...
  systemd.services.libvirt-default-network = {
    ...
    script = ''
      ...
      # net-define matches by uuid, so once migrated to our pinned uuid it just updates in place.
      # A foreign 'default' (libvirt's stock one, different uuid) won't match and net-define errors.
      # So tear it down once and redefine. All tolerant = idempotent.
