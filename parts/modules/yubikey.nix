...
lib.mkIf config.host.features.yubikey {
  ...
  # yubikey + fprint + password. swaylock=lockscreen; login=TTY.
  # greetd inherits it via substack, so overriding greetd directly makes orphan rules with no `order`.
  security.pam.services = lib.mkMerge [
    (lib.genAttrs [ "sudo" "polkit-1" "swaylock" "login" ] (svc: {
      u2fAuth = true;
  ...
  # Two-agent: default agent stays empty (local sudo = full u2f+fprint+password); FIDO keys live in this second agent, forwarded out so only remote sudo authenticates by touch
