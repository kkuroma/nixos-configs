...
- Portless services with no vhost (`clamav.nix`) follow the same self-declaring pattern with a plain `host.<name>` option, staying out of `host.services` and `knownServices`.
