...
let
  ...
  # Standalone Forgejo theme, placed at {customDir}/public/assets/css/theme-natsumikan.css and declared in ui.THEMES so Forgejo registers it
  ...
  # Haruhana, a Material Design 3 palette (config/noctalia/palettes/Haruhana.json) mapped to Forgejo vars.
  # Google Sans Flex UI, Maple Mono code, ANSI + chroma from the palette terminal block.
  ...
  # Script run as root in forgejo.service ExecStartPre.
  # Creates dirs and symlinks into the Nix store so they auto-update on rebuild without a separate service.
...
lib.mkIf (cfg != null && cfg.enable) {
  ...
  # forgejo-secrets.service only writes if files are empty, and sops secrets never are, so it's a no-op.
  # Clear ReadWritePaths so it doesn't need customDir to exist first.
  ...
  # only wllo ssh over tailscale, minimal attack surface
  ...
  services.gitea-actions-runner.instances.${config.networking.hostName} = {
    ...
    # basic packages for runner to allow simple actions and contain nix
