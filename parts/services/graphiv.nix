...
lib.mkIf (cfg != null && cfg.enable) {
  ...
  # Second, READ-ONLY instance for the public vhost: GRAPHIV_READONLY=1 registers no MCP tools and only the read webui routes.
  # Mutation is absent from the process the tunnel can reach, so a forgotten caddy matcher can't expose anything.
  # No PG needed (serves run artifacts from disk).
  ...
  services.caddy.virtualHosts = {
    # Internal (tailnet) vhost -> FULL instance. Not auto-emitted by the services glue
    # (host sets internal = false): the MCP server's DNS-rebinding guard rejects any
    # Host it isn't bound to (returns 421 "Invalid Host header"), so we must rewrite
    # the upstream Host to its dial address — the glue's one-liner reverse_proxy can't.
